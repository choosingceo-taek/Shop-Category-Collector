...
- **실측은 패널 안에서 한다**(v1.66, 사용자 요청 "F12 누르는 것도 없애자").
  DevTools에 스크립트를 붙여넣는 절차가 비개발자에겐 매번 실패 지점이었다.
  - **🔍 Test**(담기 카드 아래): 지금 보는 페이지를 **진짜 엔진**(`scrapeList`)으로
    읽어 상품 수 · 이름/사진/가격이 빈 개수 · 샘플 1건을 평문으로 보여 준다.
    스캔이 실제로 담을 값과 같은 경로라 "테스트는 되는데 스캔은 다름"이 없다.
  - **🔍 Check all**(Sites 카드 도구): 리스트 전체를 탭 하나로 순회하며 같은
    판정을 사이트마다 한 줄로 출력(초록=완전 / 노랑=일부 결측 / 빨강=0개).
    **아무것도 저장하지 않는다** — 카탈로그도 엑셀도 건드리지 않으므로 정리 전
    리스트에 그대로 돌려도 트렌드 데이터가 오염되지 않는다. 중간 Stop 가능.
  - 두 결과 모두 **Copy report**로 넘길 수 있고, 클립보드가 막히면 같은 텍스트가
    선택 가능한 textarea로 나온다(권한 없는 환경에서 조용히 실패하지 않게).
  - 레이지 그리드는 프로브가 렌더된 것만 세므로 실제 스캔보다 적게 나온다.
    믿을 신호는 정확한 수가 아니라 **0이냐 아니냐**이고, UI가 그렇게 말한다.
  - 콘솔용 `diagnose-*.js`는 남겨 둔다(어댑터 개발 시 더 깊은 덤프가 필요할 때).
