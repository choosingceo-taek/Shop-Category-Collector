...
- **사이트 뚫기는 개발자 도구로 한다 — 패널에 넣지 않는다**(v1.66.1, 사용자 요청).
  디자이너가 쓰는 화면에 "누르면 안 되는 버튼"이 있는 것이 버튼이 없는 것보다 나쁘다.
  - `content.js`의 **`probe` 메시지**: 진짜 엔진(`scrapeList`)으로 현재 페이지를
    읽어 상품 수 · 이름/사진/가격이 빈 개수 · 샘플 3건 · 플랫폼 힌트를 돌려준다.
    **읽기 전용** — 카탈로그·job·queue 어디에도 쓰지 않는다(테스트로 고정).
  - **`devcheck.js`**: 확장 **서비스워커 콘솔**(chrome://extensions → service
    worker)에 붙여넣으면 저장된 리스트를 탭 하나로 순회하며 같은 판정을 낸다.
    `devcheck()` / `devcheck("리스트명")` / `devcheck(null, 20)` / `devcheckStop()`.
    ✅ ready / ⚠️ gaps / ❌ broken 로 요약하고, 실패분만 붙여넣기 좋은 텍스트로 출력.
