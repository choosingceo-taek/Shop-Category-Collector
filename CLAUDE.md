...
- UI: **사이드 패널**(툴바 아이콘 = 리서치 컴패니언) + 페이지 좌하단 FAB.
  ...
  **LAB 진입 카드가 패널에서 가장 눈에 띄는 요소**다 — 크림 바탕에서 가장 강한 마크는
  솔리드 잉크이므로 검정 카드 + 세리프 LAB + 앰버 화살표.
  패널 톤은 **따뜻한 페이퍼**(크림 바탕 · 세리프 디스플레이 · 라운드 카드 · 필 버튼).
  레퍼런스는 감도만 차용하고 레이아웃은 우리 것이다. 하루 종일 열려 있는 패널이
  희고 시끄러운 쇼핑몰 페이지 옆에 붙으므로, 따뜻한 바탕이 "내 작업 공간"과
  "보고 있는 사이트"를 갈라 준다. 순수한 검정은 액션에만 쓴다.
