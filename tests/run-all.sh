...
PLAIN="encoding-test price-test matrix-probe dashboard-e2e"
BROWSER="goal-e2e edge-e2e panelread-e2e updnote-e2e menuimport-e2e databox-e2e selfupd-e2e"
...
for t in $BROWSER; do
  printf '%-18s ' "$t"
  rm -rf /tmp/pw-goal /tmp/pw-edge /tmp/pw-panelread /tmp/pw-updnote-e2e /tmp/pw-menuimport /tmp/pw-databox /tmp/pw-selfupd
