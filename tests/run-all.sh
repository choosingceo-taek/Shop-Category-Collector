...
PLAIN="encoding-test price-test naming-test colour-test matrix-probe dashboard-e2e"
BROWSER="goal-e2e fibreblocks-e2e rescan-e2e shortgrid-e2e assetfetch-e2e edge-e2e panelread-e2e tabs-e2e updnote-e2e menuimport-e2e databox-e2e labtools-e2e selfupd-e2e"
...
for t in $BROWSER; do
  printf '%-18s ' "$t"
  rm -rf /tmp/pw-goal /tmp/pw-fibre /tmp/pw-rescan /tmp/pw-shortgrid /tmp/pw-assetfetch /tmp/pw-edge /tmp/pw-panelread /tmp/pw-tabs /tmp/pw-updnote-e2e /tmp/pw-menuimport /tmp/pw-databox /tmp/pw-labtools /tmp/pw-selfupd
