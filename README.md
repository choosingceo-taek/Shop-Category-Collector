...
**가장 빠른 방법 — 페이지 왼쪽 아래 스캔 버튼**
지원 사이트를 열면 화면 **왼쪽 아래에 남색 원형 버튼**이 떠 있습니다.
원하는 카테고리/검색 페이지에서 그 버튼을 누르면 바로 "Scanning this page…"와
함께 수집이 시작되고, 진행 상황·⏸ 일시정지·✕ 중지가 그 자리에서 보입니다.
(옵션·필터는 마지막에 팝업에서 설정한 값을 그대로 사용)
